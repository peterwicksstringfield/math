...
TEST(ErrorHandlingScalar, is_positive_finite) {
  using stan::math::is_positive_finite;
  EXPECT_TRUE(is_positive_finite(3.0));
  EXPECT_TRUE(is_positive_finite(std::vector<double>{3, 2}));
  EXPECT_TRUE(is_positive_finite(std::vector<int>{3, 2}));
  Eigen::MatrixXd m = Eigen::MatrixXd::Constant(3, 2, 1);
  EXPECT_TRUE(is_positive_finite(std::vector<Eigen::MatrixXd>{m, m}));
  ...
  EXPECT_FALSE(is_positive_finite(std::vector<double>{3, -2}));
  Eigen::MatrixXd m2 = m;
  m2(1, 1) = -0.5;
